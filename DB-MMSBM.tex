...
\begin{tabular}{|B{0.08\textwidth}|C{0.45\textwidth}|C{0.45\textwidth}|}
\hline
 & \centering\arraybackslash \textbf{Static} & \centering\arraybackslash \textbf{Dynamic} \\
\hline
\parbox[c][0.16\textheight][c]{\linewidth}{\centering \textbf{Projected}} &
\begin{minipage}[c][0.16\textheight][c]{\linewidth}
\centering
\begin{tabular}{@{}p{0.48\linewidth}p{0.48\linewidth}@{}}
Cho \& Fowler (2010) & Cranmer \& Desmarais (2011)\\
Franzese et al.\ (2012) & Bodea \& Hicks (2015)\\
Nyhan \& Montgomery (2015) & Goddard (2018)\\
Weschle (2018) & Boucher \& Thies (2019)\\
Box-Steffensmeier et al.\ (2019) & Kim et al.\ (2019)\\
Weschle (2019) & Aar{\o}e \& Peterson (2020)\\
Battaglini et al.\ (2020) & Box-Steffensmeier et al.\ (2020)\\
Cruz et al.\ (2020) & Jiang \& Zeng (2020)\\
Siegel \& Badaan (2020) & Naidu et al.\ (2021)\\
Blair et al.\ (2022) & Fishman \& Davis (2022)\\
Shaffer (2022) & Abi-Hassan et al.\ (2023)\\
Lo et al.\ (2023) & Edgerton (2024)\\
Oklobdzija (2024) & \\
\end{tabular}
\end{minipage}
&
...
\centering
\begin{tabular}{@{}p{0.48\linewidth}p{0.48\linewidth}@{}}
Gilardi et al.\ (2020) & Kim et al.\ (2020)\\
Nieman et al.\ (2021) & Harden et al.\ (2023)\\
Uppala \& Desmarais (2023) & \\
\end{tabular}
\end{minipage}
\\
\hline
\parbox[c][0.16\textheight][c]{\linewidth}{\centering \textbf{Bipartite}} &
\begin{minipage}[c][0.16\textheight][c]{\linewidth}
\centering
\begin{tabular}{@{}p{0.48\linewidth}p{0.48\linewidth}@{}}
Kim \& Kunisky (2020) & Sweet (2021)\\
Lo et al.\ (2025) & \\
\end{tabular}
\end{minipage}
&
...
In international relations, scholars often posit theories about the effects of latent groups of actors on relational outcomes of interest over time. In the study of inter-governmental organizations (IGOs), scholars have long posited how IO membership as a structured system of interconnections shape states' foreign and domestic policy pursuits \citep{beckfield2003inequality, lupu2017networked, davis2023discriminatory}. Understanding which countries join which IGOs, therefore, is crucial for evaluating these effects.

The country-IGO system of interconnections is an example of a \emph{dynamic bipartite network} --- one in which there are two node types, ties that evolve over time only form across types (rather than within them). In fact, many political networks are both bipartite and dynamic. The connections between states and treaties, legislators and bills, and lobbyists and politicians are all examples of bipartite networks in which ties form and dissolve as a result of strategic considerations.

Despite how common these dynamic bipartite structures are, conventional models often rely on network projections or simplified static representations that distort the true structure of interactions. Table~\ref{table:lit_review} summarizes the methods used in studies published in top journals in Political Science between 2010 and 2024 when the underlying data structure could be most adequately represented using a dynamic bipartite model. None of these uses an approach that correctly captures both the dynamic and bipartite nature of the networks, risking reaching incorrect conclusions driven by temporal and cross-sectional aggregation bias. 

To address these issues and better understand the evolution of country IO memberships, we propose a \textbf{dynamic bipartite mixed-membership stochastic blockmodel (DB-MMSBM)} regression. Our approach enables researchers to examine changing group dynamics in bipartite networks over time while avoiding two key pitfalls: (1) the artificial inflation of clustering patterns resulting from failing to account for the bipartite nature of data, and (2) the loss of valuable temporal information when networks are treated as static. By modeling bipartite networks dynamically, \textbf{DB-MMSBM} identifies latent structures that better reflect the evolving nature of international cooperation. Scholars can apply our model in broad contexts where relational ties are formed between two different types of actors.

In what follows, we introduce the state-IO network data, which is central to the study of international cooperation. We then  describe the intuition behind our proposed model, formalize that intuition, and fit it to yearly state–IO membership data  covering 200 countries and over 400 IGOs. We find that substantial nuance in the strategic drivers of IO membership --- nuance that would be lost entirely under alternative existing approaches. Specifically, our model allows us to understand how both regime type and policy stances shape decisions to cooperate on the basis of shared economic and security interests, and how characteristics of the IOs themselves (such as democratic density among members) can help drive these partnerships.     

\FloatBarrier
\section{The State-IO Membership Network}

As our motivating example, we highlight the bipartite and dynamic nature of the IO membership data and discuss how common practices of projecting or collapsing these network data in the existing literature can lead to an overemphasis of regime type in the role of states' IO memberships, as well as an incomplete understanding of the network's hierarchical structure.

\subsection{Background}

Studying the structure of the state-IO network is a topic of great interest to international relations scholars and policy pundits. On the one hand, states frequently use these organizations to overcome informational hindrance to cooperation, signal and commit to policy stances, and benefit from the club goods provided by IOs \citep{keohane1984after, simmons2010credible, poast2019organizing, carnegie2020secrets}. On the other hand, many IOs manage their membership composition through accession and agenda setting, in order to maintain preference congruence, facilitate policy outputs, and ensure organizational vitality \citep{downs1998managing, gray2018life, davis2023discriminatory}. The dynamic interaction between states and IOs, as a result, has formed the foundational structure of international cooperation over time.

Consequently, scholars have been interested in establishing the determinants of membership ties between states and IOs. A prominent argument in the literature of international cooperation identifies regime type as a crucial driver in whether and how states participate in international cooperation. On the one hand, democracies and autocracies differ in their tendencies to join IOs. Given their domestic institutional constraints, democracies are better at conveying their policy commitment, and join trade, climate, and human rights institutions with each other at a higher rate than their autocratic counterparts \citep{mansfield2000free, moravcsik2000origins, battig2009national}. Conversely, while autocracies are less likely to engage in international cooperation, they are more likely to do so with other autocracies \citep{leeds1999domestic}. On the other hand, countries transitioning across regime types may join IOs to safeguard their domestic institutions and prevent regime reversals. Across democracies and autocracies, studies have argued that joining IOs more densely populated by states with similar political institutions helps a nascent regime to survive and consolidate \citep{pevehouse2005democracy, poast2019organizing, debre2022clubs}. 

In addition to states' cooperation patterns, more recent studies have also investigated how features of IOs shape interactions between states and IOs. Mirroring the focus on regime type in the state-centric studies, scholars posit that \textit{democratic density} -- the share of democratic members within an IO -- affect how IOs behave. When establishing IOs, democracies and autocracies have different preferences towards the institutional design of IOs \citep{koremenos2016continent}. As a result, IOs with higher levels of democratic density are more likely to have more independence and delegated authority \citep{tallberg2025democracy}, operate based on majority voting \citep{hooghe2019theory}, and more open to non-state actors' participation \citep{tallberg2014explaining, tallberg2016democratic}. These design features, in turn, shapes IOs' effectiveness in promoting economic cooperation \citep{gray2018life} and pursuing liberal policies \citep{kreuder2022lio}. In contrast, IOs with lower democratic density face stricter controls in their bureaucracy by member states, focus more heavily on regime survival, and promote illiberal norms as ``traditional values". \citep{velasco2023transnational, cottiero2025illiberal, takahashi2025institutionalized}.

Taken together, the cleavage between democracies and autocracies -- as well as between IOs with high and low levels of democratic density -- in their preference towards and behaviors in international cooperation implies that the structure of state-IO network is clustered around regime types. In similar veins, scholars have also argued that clustering within the state-IO network can be driven by economic interdependence \citep{gilpin2016political, lupu2016states}, geopolitics \citep{pratt2018deference, davis2023discriminatory}, or shared ideology and norms \citep{barnett2012rules, voeten2021ideology}. How do these cleavages interact with one another and form the boundaries of inter-group associations between states and IOs, however, remains an open question. 

When analyzing the pattern of states' IO membership, researchers often project the bipartite network onto unipartite ones. For instance, many studies transform the state-IO network onto a unipartite graph where an edge between two states represents whether or not they share a co-membership in certain IOs (or, in the case of a weighted projection, the number of co-memberships, see \citeauthor{lupu2017networked}, \citeyear{lupu2017networked}, for example). Researchers may even do away with the network structure entirely and study only monadic attributes of states (e.g., whether democratization leads to more IO membership). In adopting this strategy,
however, researchers risk omitting heterogeneity across IOs, such as their accession stringency or membership size. Conversely, studies that project the state-IO network onto a unipartite IO network where edges indicate membership overlaps ignore heterogeneities across states.\footnote{In Table~\ref{lit_table_io}, we summarize papers published in top journals in general political science and the IR-subfield between 2005-2025 that examine state-IO networks and characterize how each study addresses the network structure.} Recent methodological studies have shown that ignoring such IO-specific or state-IO pair-specific information can lead to severe aggregation bias \citep{lo2023statistical}.

Another common methodological approach adopts the assumption of  conditional independence of state dyad-year observations given some covariates within the generalized linear model framework. However, there are reasons to believe that patterns in states' IO membership fail to meet this assumption. Temporally, both states and IOs evolve over time. States may shift between regime types, commit to different norms, or adjust their geopolitical goals. Similarly, IOs may alter their accession rules, expand or contract their operations, and become more or less salient given their issue areas. Neither states nor IOs, therefore, remain static within a given cluster or group. Dyadically, states' decision and ability to join one IO may be predicted by their memberships in other organizations \citep{haftel2022measuring}: On the one hand, major powers build influence and coalitions through membership in multiple and overlapping organizations \citep{rosecrance2001new}. On the other hand, IOs may use states' membership in other organizations to pre-screen their legibility and commitment to engage in cooperation with members in their own fora \citep{donno2015screening}. Moreover, once states have joined an IO, their participation tend to be sticky. Even states' exits and withdrawals from IOs have received increasing scholarly and media attentions, they remain extraordinarily rare \citep{von2019hello, schmidt2025damaged}. 
...
%  \centering
%  \includegraphics[width=\linewidth]{illustration/new_2/illustration.pdf}
%  
%  (a) State–IGO membership
%\end{minipage}
...
%  \centering
%  \includegraphics[width=\linewidth]{illustration/new_2/illustration_project.pdf}
%  
%  (b) Projected state–state co-membership
%\end{minipage}
%\hfill
%\begin{minipage}{0.48\linewidth}
%  \centering
%  \includegraphics[width=\linewidth]{illustration/new_2/illustration_project_io_combo.pdf}
  
%  (c) Projected state–state co-membership (ties colored by shared IO membership)
%\end{minipage}

%\caption{\small \textbf{Illustration of the membership relation between states and IGOs (2014).} Figure (a) shows the geographic distribution of membership for four major IGOs in 20 states. Each gray dot represents a state, plotted at its geographic centroid, while red triangles indicate the positions of four prominent IGOs: the United Nations (UN), the Organisation for Economic Co-operation and Development (OECD), the African Union (AU), and the Shanghai Cooperation Organization (SCO). The 20 states include seven representative states we analyze later, and 13 other randomly sampled states. Curved lines connect each state to the organizations in which it holds membership. Figure (b) shows the same network projected into a unipartite network. Ties between states indicate the number of shared co-membership. This figure highlights the loss of information when projecting the state-IGO co-membership network into a unipartite one.}
%\caption{\small \textbf{Illustration of the membership relation between states and IGOs (2014).} Figure (a) shows the geographic distribution of membership for five major IGOs in 20 states. On the top panel, each node is an IO, with its size scaled by the number of ties: the United Nations (UN), the African Union (AU), the Organisation for Economic Co-operation and Development (OECD), the Organization of American States (OAS), and the Shanghai Cooperation Organization (SCO), On the lower panel, each gray dot represents a state, with its size scaled by the number of ties. The 20 states include seven representative states we analyze later, and 13 other randomly sampled states. Lines connect each state to the 
%organizations in which it holds membership. Figure (b) shows the same network projected into a unipartite network. Ties between states indicate the number of shared co-membership. Figure (c) shows the same network as (b), with ties colored by the shared IO membership profile between two states. This figure highlights the loss of information when projecting the state-IGO co-membership network into a unipartite one.}
%\label{map}
%\end{figure}
...
Figure~\ref{net_visual} illustrates part of this network, moving from 1965 to 2014. We focus on a subset of four IOs and 14 states. %First, different IOs have distinct numbers and types of members. For example, the OECD (Organization for Economic Co-operation and Development) has more members than NATO (North Atlantic Treaty Organization). Both IOs are also more democratic than APEC (Asia-Pacific Economic Cooperation). States also join If we project this network onto a unipartite one, we would lose the difference in their membership compositions, which is important for understanding the logic of cooperation within IOs. 
The first column shows the actual bipartite state-IGO network. It has undergone significant evolution over time. Some IGOs came into existence only recently (e.g., G24 and APEC), while others have been established earlier (e.g., the OECD and NATO). The network has become denser over time, indicating that states' membership in IGOs has expanded. The trajectory of change also varies across IGOs. Within our selected subset of states, APEC (Asia-Pacific Economic Cooperation) has expanded its membership rapidly in recent years, especially towards many autocratic countries. G24 members, in contrast, have become more democratic. States also differ markedly in the breadth and timing of their institutional engagements: some finalized their ties in the early decades, while others are still actively joining IGOs (though the latter still differ in which organizations they join). Collapsing this network into a static one or slicing it into several snapshots would miss the changing behavior of states and the dynamic structure of IGOs. 

If we simply project this bipartite network onto a unipartite one, we would lose significant information and thus draw wrong conclusions. The second column of Figure~\ref{net_visual} counts the number of shared IGO co-memberships between two states, thus projecting the corresponding bipartite network onto a unipartite one. In 2014, the network appears to consist of two groups of states. One group of states is tightly bound together, including France, Italy, Turkey, the U.S., Canada, Japan, Mexico, the Philippines, and Peru. Another group of states is loosely connected to one another, including Russia, India, Vietnam, China, and Malaysia. One would conclude that the first group is a club of states with denser institutional cooperation among them, and the second group is also cooperating but at a lower level. 

However, this conclusion is problematic because it ignores the drastic difference in the specific IGOs the two states are joining. The third column of Figure~\ref{net_visual} colors the edges in the simple projected networks by the shared IO combination between each state dyad. This reveals that there are actually multiple types of cooperation within each group, which was masked by the projection. Among states with two (and three) shared IGO co-memberships, the blue group (France, Italy, Turkey, Canada, and the U.S.) shared membership in NATO and the OECD. The two IOs are highly selective, and members cooperate on high-stakes issues. Therefore, these states form a group of allies that cooperates closely on both security and economic affairs. The green group (Canada, the U.S., Mexico, and Japan) emerged in 1990. States in this group shared membership in APEC and the OECD, which comprises large democracies (mostly developed countries) that cooperate on economic issues. The purple group that emerged in 2014 (Mexico, the Philippines, and Peru) includes states that are members of both APEC and G24 (Group of Twenty-Four). This is a group of developing countries cooperating on economic issues. Similarly, among those that share only one co-membership, some advanced economies cooperate on economic affairs within the OECD, others work on issues related to developing economies within the G24, and others are clustered in APEC.

Therefore, collapsing IGOs and focusing only on the number of organizations that states join or ignoring the temporal development of this network would miss important structural and behavioral variation. Such simplification obscures the mechanisms through which cooperation occurs, treating all organizations as equivalent and all membership ties as static. In reality, both states and IGOs differ substantially in when and how they form connections, and these differences are often associated with regime type, political alignment, and institutional design. By modeling the dynamic bipartite structure directly, we retain the full heterogeneity in membership composition and timing, allowing us to identify how distinct types of states and organizations evolve over time.
...
%  {illustration/new_3/bipartite1.pdf}&
%          \includegraphics[width=0.33\linewidth]
%  {illustration/new_3/proj1.pdf} &
%  \includegraphics[width=0.33\linewidth]
%  {illustration/new_3/projco1.pdf}\\ 
%     \includegraphics[width=0.33\linewidth]
%  {illustration/new_3/bipartite2.pdf}&
%          \includegraphics[width=0.33\linewidth]
%  {illustration/new_3/proj2.pdf} &
%  \includegraphics[width=0.33\linewidth]
%  {illustration/new_3/projco2.pdf}\\ 
%      \includegraphics[width=0.33\linewidth]
%  {illustration/new_3/bipartite3.pdf}&
%          \includegraphics[width=0.33\linewidth]
%  {illustration/new_3/proj3.pdf} &
%  \includegraphics[width=0.33\linewidth]
  %{illustration/new_3/projco3.pdf}\\ 
 %  \end{tabular}
 % \caption{State-IGO membership networks (1965-2014)}\label{}
 % \end{center}
 %  Note: 
%\end{figure}
...
\includegraphics[width=1\linewidth]{illustration/new_4/full5.pdf} \\[-2pt]
\end{tabular}
\caption{\small \textbf{State-IGO membership networks (1965-2014).} The figure depicts three snapshots of the dynamic bipartite membership network between 14 states and four IGOs. The first column shows the real bipartite data structure. The second column shows the simple projection of the corresponding, with edges indicating the number of IO co-membership between two states. The third column shows the network in the second column with edges colored by the combination of IOs that two states share membership in. The four IGOs are the OECD (Organization for Economic Co-operation and Development), G24 (Group of Twenty-Four), NATO (North Atlantic Treaty Organization), and APEC (Asia-Pacific Economic Cooperation). Both IGO and state node sizes are scaled by the number of ties they have. These networks illustrate how networks change drastically over time, and that information gets lost when projecting a bipartite networks into a unipartite ones.}
\label{net_visual}
\end{figure}
...
Analyzing patterns of international cooperation within the state-IGO network requires a model that defines the probability of cooperation as a function of memberships in latent groups both in countries and IGOs. Methodologically, the model needs to simultaneously uncover how these latent memberships change over time and retain the bipartite structure of the network, which contains the information of country- and IGO-level characteristics -- like regime types for countries and democratic density for IGOs. We extend \citet{lo2023statistical}, a static bipartite MMSBM to a dynamic one. Our model is also related in spirit to methods such as Social Influence Regression \citep{minhas2024decomposing}, which uses covariates to explain autoregressive dyadic influence in unipartite networks. However, our method applies to bipartite networks and models latent group structure instead of dyadic influence.


In this section, we describe the dynamic bipartite mixed-membership stochastic blockmodel (DB-MMSBM). We then formally present the full modeling approach, and discuss a fast estimation algorithm that enables the analysis of large networks.

\subsection{Modeling strategy}
We represent an observed network as a bipartite graph that evolves over time. The network consists of two disjoint sets or families of nodes. For each period, a set of undirected edges are formed between pairs of nodes belonging to these two different families. In other words, no edge exists between any two nodes of the same family. In our application, states and IGOs correspond to these two families. The edges represent states holding formal membership in IGOs, which only occur between states and IGOs and do not exist directly among states or IGOs.

The dynamic bipartite mixed-membership stochastic blockmodel (DB-MMSBM) assumes that a node belongs to one of several latent groups when interacting with nodes of the other family. For any dyadic relationship between two nodes of different families, the latent group memberships of the nodes determine the likelihood of forming an edge between them. Thus, a state may belong to different latent communities when deciding whether to
join different IGOs. Similarly, IGOs can be sorted into separate latent groups across state-IO dyads. For instance, China might have behaved similarly to other democracies when deciding whether to join the World Trade Organization (WTO), but might have acted differently when considering membership in the Shanghai Cooperation Organization (SCO). This is a pattern that could help us understand China's unique role in institutional cooperation, simultaneously engaging with established liberal regimes and fostering parallel institutions aligned with its regional and strategic interests.
...
portion of the model. Mixed-memberships can be explained using node-level covariates, which are characteristics of states and IGOs, such as regime type for states and membership size for IGOs.

Then a matrix, which is the so-called blockmodel, encodes the probabilities
of forming an edge (i.e., state membership in IGOs) between communities of states and communities of IGOs. Some pairs of communities may have a higher probability of establishing membership than others. The blockmodel can thus capture a variety of strategies in joining IGOs among states, affording them flexibility in the way they cooperate with others in the international fora. 
...
To improve the scalability of our proposed approach, we adopt two complementary strategies. First, we rely on a mean-field variational approximation to the collapsed posterior in Equation~\ref{eq2}, which first defines a factorized distribution $\tilde{Q}_{\mathbf{L}}$ over latent variables $\mathbf{L} = \{\mathbf{Z}, \mathbf{U}, \mathbf{S}\}$ and a corresponding lower bound $\mathcal{L}$ for the collapsed posterior. We then optimize this lower bound with respect to the variational parameters to approximate the true posterior over our latent variables 
\citep{teh2006collapsed}. Using an EM algorithm, we iterate between finding an optimal $\tilde{Q}_{\mathbf{L}}$ (the E-step) and optimizing the corresponding lower bound with respect to the hyper-parameters $\mathbf{B}$, $\boldsymbol{\beta}$, and $\boldsymbol{\gamma}$ (the M-step). It has been shown that the type of marginalization scheme we implement improves the quality of the variational approximation in related contexts \citep{jordan1999introduction, lo2023statistical}. 

Second,  we rely on stochastic optimization to find the maximum of the lower bound. With decreasing step sizes, our algorithm follows a noisy estimate of the gradient of $\mathcal{L}$ formed by subsampling dyads in the original network. When using the correct schedule for the step-sizes, this procedure is guaranteed to find a local optimum of the lower bound without the need to perform a costly update over the parameters associated with all dyads at every iteration \citep{gopalan2013efficient}. Details of our exact estimation procedures — including the formal procedure of our estimation algorithm, the required gradients and hessian computations, initial values for all relevant parameters and latent variables, and sub-network sampling strategy  — are available in Appendix~\ref{appendix:em}.
...
&+s_{t m} \sum_{q \in V_{2t}}\left[\frac{\Gamma\left(\xi_{q t m}\right)}{\Gamma\left(\xi_{q t m}+ N_{1t}\right)}\right] + s_{t m} \sum_{q \in V_{t2}} \sum_{h=1}^{K_2} \mathbb{E}_{\tilde{Q}_2}\left[\log \left[\frac{\Gamma\left(\alpha_{q t m h}+C_{q t h}\right)}{\Gamma\left(\alpha_{q t m h}\right)}\right]\right] +\text { const } \end{aligned}
\end{equation*}
...
During the M-step, we find locally optimal values of $\mathbf{B}$, $\boldsymbol{\beta}$, and $\boldsymbol{\gamma}$ with respect
to the lower bound in Equation~\ref{eq:lb}, using a quasi-Newton method.\footnote{To regularize the fit, we define independent standard Normal priors for all parameters. When required, standard errors for these quantities are obtained by first sampling from the approximate posteriors of the latent variables, and then obtaining expected values of the
log-posterior Hessian evaluated at the approximate MAP estimates.} We start by updating the value of $\mathbf{B}$ by collecting the terms that contain $B_{gh}$ in Equation~\ref{eq:lb} for the lower bound expression:
...
Collect the terms that contain $\boldsymbol{\gamma}$ in the lower bound (note that $\theta_{pqtgh} = \mathrm{logit}^{-1} (B_{z_{pqt},u_{pqt}} + \mathbf{d}_{pq}^\top \boldsymbol{\gamma})$ is also a function of $\boldsymbol{\gamma}$), $J_{d}$ is the number of dyadic covariates:
...
Like other stochastic VI (SVI) algorithms, ours follows a random gradient with expected value equal to the true gradient of the lower bound in Equation~\ref{eq:lb}. To form this unbiased gradient, we use the dyad sampling scheme proposed by \cite{dulac2020mixed} to sample a mini-batch of nodes within each time period $t$, and form subgraphs $\mathbf{Y}_t^{(s)}$ at each step of the algorithm. The algorithm proceeds by optimizing the local variational parameters (i.e. $\boldsymbol{\Lambda}$ and $\boldsymbol{\Delta}$) for all dyads $(p, q)$ in each sub-network $\mathbf{Y}_t^{(s)}$ using the updates given in the previous section, holding global counts constant at their most current values. We then condition on these locally updated variational parameters and obtain an intermediate value of all global counts (i.e. $\mathbf{C}$ and $\mathbf{U}$) by computing their expected value under the mini-batch sampling distribution:
...
We set the step-size $\rho_s$ = $(\tau + s)^{-p}$, and $p \in (0.5, 1.0]$, $\tau \geq 0$ are researcher-set arguments controlling the extent to which previous iterations affect current values of the sufficient statistics \citep{cappe2009line, hoffman2013stochastic}. To set the values
of our hyperparameters, we once again follow an empirical Bayes approach, updating the hyper-parameters along with the global sufficient statistics by taking a step in the direction of the gradient of the stochastic lower bound \citep{hoffman2013stochastic}. As an example, for $\boldsymbol{\gamma}$, we have: 

$$
\gamma^{(s)}=\gamma^{(s-1)}+\rho_{s} \nabla_{\gamma} \mathcal{L}_{\hat{\phi}, \hat{\lambda}, \hat{\delta}}^{(s)}(\gamma)
$$
where the gradient is given in Section~\ref{appendix:em}. The updates for all other hyperparameters are similarly defined.
